...
void KEY_GPIO_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(GPIO_KEY1_CLK | GPIO_KEY2_CLK | GPIO_USERKEY_CLK, ENABLE);
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN; 
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_USERKEY_PIN;
	GPIO_Init(GPIO_USERKEY_PORT, &GPIO_InitStructure);		
}
...
uint8_t Get_Key(void) 
{
	...
	if(!GPIO_ReadInputDataBit(GPIO_USERKEY_PORT,GPIO_USERKEY_PIN))
	{
			ReadKey |= PRESS_USERKEY;
	}
